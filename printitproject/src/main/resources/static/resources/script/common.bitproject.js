function getCurrentDateTime(format){
    ...
    var month = today.getMonth()+1;   // get month --> array [0-11]
    if(month<10) month = "0"+month;  // yyyy-mm-dd
    var date = today.getDate();         // get date --> range [1-31]
    if(date<10) date = "0"+date; // yyyy-mm-dd

    var hours = today.getHours(); //[0-23]
    if (hours < 10) hours = "0" +hours;
    var minuntes = today.getMinutes();
    if (minuntes < 10) minuntes = "0" +minuntes;


    var currentdate = today.getFullYear()+"-"+month+"-"+date;
    var currenttime = hours+":"+minuntes;
    var currentdatetime = today.getFullYear()+"-"+month+"-"+date+"T"+hours+":"+minuntes;
...
function table(parentid,id,metadata, hasBtn=true){
    ...
    for(var i=1; i<=metadata.length; i++){
        ...
        //fa fa-sort isort
        th.innerHTML = metadata[i-1]['name'] + "   " + '<i class="" aria-hidden="true"></i>';
        row1.appendChild(th);
        var th = createElement('th');
...
function tableinvoice(parentid,id,metadata, hasBtn=true){
    var parent = document.getElementById(parentid);
    var table = createElement('table',id);
    table.border = 2;
    table.classList.add('table');
...
function designercomboBoxBinder(field,pattern,obj,prop,oldobj){
    field.setAttribute('data-object',obj);
    field.setAttribute('data-oldobject',oldobj);
    var ob = window[field.getAttribute('data-object')];
    var oldob = window[field.getAttribute('data-oldobject')];
...
//File Chooser Functions2

function photoBinder(field,pattern,obj,prop,oldobj, imgViewPhoto,picname,picnamepro){
    field.setAttribute('data-object',obj);
    field.setAttribute('data-oldobject',oldobj);
    var ob = window[field.getAttribute('data-object')]
    var oldob = window[field.getAttribute('data-oldobject')]
    ...
    reader.onload = function() {
        ob[prop] = btoa(reader.result); //get result of that reader by onload and binder into our property
        imgViewPhoto.style.display = "block";
        imgViewPhoto.src = reader.result;

       var imagename = field.files[0].name;
        ob[picnamepro] = imagename;
       picname.value = imagename;
        picname.style.border = valid;
        picname.disabled = false;
    }
